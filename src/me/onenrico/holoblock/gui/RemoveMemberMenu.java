...
public class RemoveMemberMenu {
	...
	public static void open(Player player,String rawloc,String member) {
		...
		String title = pu.t(ConfigPlugin.getStr("RemoveMemberMenu.Title", "Title &cNot Configured !"));
		...
		for(int r = 0;r < 6;r++) {
			for(int x = 0;x < 4;x++) {
				if (r == 0) {
					InventoryUT.setItem(inv, x, YesItem)
					.addClick("MinusMember:"+rawloc+"<<"+member);
				}else {
					int y = x + (r * 9);
					InventoryUT.setItem(inv, y, YesItem)
					.addClick("MinusMember:"+rawloc+"<<"+member);
				}
			}
			for(int x = 5;x < 9;x++) {
				if (r == 0) {
					InventoryUT.setItem(inv, x, NoItem)
					.addClick("EditMemberMenu:"+rawloc);
				}else {
					int y = x + (r * 9);
					InventoryUT.setItem(inv, y, NoItem)
					.addClick("EditMemberMenu:"+rawloc);
				}
			}
		}
