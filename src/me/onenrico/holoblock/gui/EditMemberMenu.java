...
public class EditMemberMenu {
	...
	public static void open(Player player,String rawloc,int page) {
		setup();
		int max = ConfigPlugin.getMaxMember(player,Seriloc.Deserialize(rawloc).getWorld());
		...
		String title = pu.t(ConfigPlugin.getStr("EditMemberMenu.Title", "Title &cNot Configured !"));
		...
		if(page > 1) {
			...
			.addClick("OpenPageMember:"+rawloc+":"+(page - 1));
		...
		for(int x = 0;x<7;x++) {
			InventoryUT.setItem(inv, x + 46, AddMemberItem)
			.addClick("AddMember:"+rawloc);
		}
